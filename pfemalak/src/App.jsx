...
function App() {
  const [view, setView] = useState(
    window.location.hash === '#/admin-access' ? 'admin-login' : 'home'
  );
...
{view === 'admin-login' && (
        <AdminLogin onLoginSuccess={(user) => { setCurrentUser(user); setView('dashboard'); window.location.hash = ''; }} />
      )}
      {view === 'login' && <Login lang={lang} onBack={() => setView('home')} onSignup={() => setView('inscription')} onLoginSuccess={(user) => { setCurrentUser(user); setView('dashboard'); }} />}
      {view === 'inscription' && <Inscription lang={lang} onBack={() => setView('home')} onLogin={() => setView('login')} />}

      {view === 'dashboard' && currentUser?.role === 'admin' && (
        <AdminPage user={currentUser} onLogout={() => { setCurrentUser(null); setView('home'); }} />
      )}
      {view === 'dashboard' && currentUser?.role === 'agent' && (
        <AgentPage user={currentUser} onLogout={() => { setCurrentUser(null); setView('home'); }} />
      )}
      ...
      {view !== 'login' && view !== 'inscription' && view !== 'dashboard' && view !== 'admin-login' && <Footer lang={lang} />}
    </div>
  );
}
