...
import carSafetyAnimation from './car-safety.json';
import backgroundAnimation from '../animations/anim.json';
import hhAnimation from '../animations/hh.json';
import {
...
const StatusBadge = ({ status }) => {
  const map = {
    upcoming: { label: 'En attente', cls: 'badge-upcoming', Icon: AlertCircle },
    confirmed: { label: 'Confirmé', cls: 'badge-completed', Icon: CheckCircle },
    refused: { label: 'Refusé', cls: 'badge-cancelled', Icon: XCircle },
...
const ClientDashboard = ({ user, lang = 'FR', onLogout, onNavigate }) => {
  ...
  const fetchDbParkings = useCallback(async () => {
    try {
      const data = await fetch('http://localhost:8000/api/parkings').then(r => r.json());
      if (Array.isArray(data)) setDbParkings(data);
    } catch (err) {
      console.error("Failed to fetch parkings from DB", err);
    }
  }, []);

  const fetchParkingZones = useCallback(async (id) => {
    setZonesLoading(true);
    try {
      const data = await fetch(`http://localhost:8000/api/parkings/${id}/zones`).then(r => r.json());
      if (Array.isArray(data)) {
        setDynamicZones(data);
        if (data.length > 0) {
          setForm(f => ({ ...f, zone: data[0].name }));
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    fetchReservations();
    fetchDbParkings();
  }, [fetchVehicles, fetchReservations, fetchDbParkings]);

  useEffect(() => {
    if (form.parkingId && (typeof form.parkingId === 'number' || !isNaN(parseInt(form.parkingId)))) {
      fetchParkingZones(form.parkingId);
    }
  }, [form.parkingId, fetchParkingZones]);

  /* ── City/Parking computed ── */
  const CITY_LIST = [...new Set(dbParkings.map(p => p.city))];
  if (CITY_LIST.length === 0) CITY_LIST.push('Casablanca', 'Rabat', 'Marrakech', 'Tanger');

  const cityParkings = dbParkings.length > 0 
    ? dbParkings.filter(p => p.city === form.city) 
    : [];
  ...
  const handleGeolocate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        const allParkings = dbParkings.map(p => ({
            ...p,
            lat: p.latitude,
            lng: p.longitude,
            free: p.free_spots,
            total: p.total_spots,
            distKm: haversine(lat, lng, p.latitude, p.longitude),
                  ...
                  <div className="select-wrap city-select">
                    <select
                      ...
                      onChange={e => {
                        ...
                        const firstParking = dbParkings.find(p => p.city === city)?.id || '';
            ...
            <div className="card parking-card">
              ...
              <div className="zone-tabs">
                {dynamicZones.map(z => (
                  <button
                    key={z.id}
                    className={`zone-tab ${form.zone === z.name ? 'active' : ''}`}
                    onClick={() => { setForm(f => ({ ...f, zone: z.name })); setSelectedSpot(null); }}
                  >
                    {z.name}
                  </button>
                ))}
              ...
              {/* Spot grid */}
              <div className="parking-grid-scroll">
                <div className="parking-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '1rem', padding: '1rem' }}>
                  {zonesLoading ? (
                    <p style={{ padding: '2rem', textAlign: 'center', width: '100%' }}>Chargement du plan...</p>
                  ) : zoneSpots.length === 0 ? (
                    <p style={{ padding: '2rem', textAlign: 'center', width: '100%' }}>Aucune place configurée pour cette zone.</p>
                  ) : (
                    zoneSpots.map(s => (
                      <SpotCard
                        key={s.id}
                        spot={{ id: s.code, status: s.status, type: s.type, price: s.price_per_hour, dbId: s.id }}
                        active={true}
                        selected={selectedSpot?.dbId === s.id}
  ...
  const DashboardPage = () => {
    ...
    const upcoming = reservations.filter(r => r.status === 'upcoming' || r.status === 'confirmed');
  ...
  const ReservationsPage = () => {
    return (
      <div className="page-body">
        ...
        {/* Premium Banner with Animation when there are reservations */}
        {!resLoading && reservations.length > 0 && (
          <div className="reservations-premium-banner">
            <div className="rpb-left">
              <span className="rpb-tag">ParLak Sécurisé</span>
              <h3>Votre stationnement est entre de bonnes mains</h3>
              <p>Retrouvez ci-dessous l'historique complet ainsi que le statut en direct de vos places de parking réservées. Présentez votre code QR à l'entrée du parking.</p>
              <div className="rpb-stats">
                <div className="rpb-stat-item">
                  <span className="rpb-stat-val">
                    {reservations.filter(r => r.status === 'upcoming' || r.status === 'confirmed').length}
                  </span>
                  <span className="rpb-stat-lbl">Actives</span>
                </div>
                <div className="rpb-stat-item">
                  <span className="rpb-stat-val">
                    {reservations.filter(r => r.status === 'completed').length}
                  </span>
                  <span className="rpb-stat-lbl">Terminées</span>
                </div>
              </div>
            </div>
            <div className="rpb-right">
              <div className="rpb-lottie-container">
                <Lottie animationData={carSafetyAnimation} loop={true} style={{ height: '100%', width: '100%' }} />
              </div>
            </div>
          </div>
        )}
        ...
        {resLoading ? (
          <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
            <p style={{ color: '#64748b' }}>Chargement de vos réservations...</p>
          </div>
        ) : reservations.length === 0 ? (
          <div className="card reservations-empty-state">
            <div className="res-empty-lottie">
              <Lottie animationData={carSafetyAnimation} loop={true} />
            </div>
            <h3>Aucune réservation active</h3>
            <p className="text-muted">
              Vous n'avez pas encore de place de parking réservée. Planifiez votre prochain trajet et réservez une place sécurisée en quelques clics !
            </p>
            <button className="btn-primary" onClick={() => setActivePage('reserve')}>
              <Plus size={16} /> Réserver une place maintenant
            </button>
          </div>
        ) : (
                      ...
                      <td>
                        <div className="tbl-actions">
                          {(r.status === 'upcoming' || r.status === 'confirmed') && (
                            <button className="icon-btn qr-btn" title="Voir QR code" onClick={() => setQrModal(r)}>
                              <QrCode size={14} />
                            </button>
                          )}
                          <button className="icon-btn" title="Détails" onClick={() => setQrModal(r)}>
                            <Eye size={14} />
                          </button>
                          <button className="icon-btn" title="Télécharger QR" onClick={() => { setQrModal(r); setAutoDownload(true); }}>
                            <Download size={14} />
                          </button>
                          {(r.status === 'upcoming' || r.status === 'confirmed') && (
                            <button className="icon-btn red" title="Annuler" onClick={() => handleCancelReservation(r.id)}>
                              <XCircle size={14} />
                            </button>
                          )}
                        </div>
  ...
  ══════════════════════════════════════════ */
  const VehiclesPage = () => (
    <div className="page-body vehicles-page-wrapper" style={{ position: 'relative', overflow: 'hidden', zIndex: 1 }}>
      {/* Background Lottie Animation for Vehicles Page */}
      <div className="vehicles-background">
        <Lottie 
          animationData={hhAnimation} 
          loop={true} 
          autoplay={true} 
      ...
      <div className="vehicles-content-layer" style={{ position: 'relative', zIndex: 2 }}>
        <div className="page-title-row">
    ...
    <div className="dash-layout">
      {activePage !== 'vehicles' && (
        <div className="dashboard-background">
          <Lottie animationData={backgroundAnimation} loop={true} autoplay={true} rendererSettings={{ preserveAspectRatio: 'xMidYMid slice' }} />
        </div>
      )}
      <Sidebar />
